...
import { parseFFCSV, getCSVPreview, validateCSVStructure } from '../ff-import';
...
describe('parseFFCSV', () => {
  ...
  it('sets source to fertility-friend', () => {
    const result = parseFFCSV(sampleCSV);

    expect(result.observations[0].source).toBe('fertility-friend');
  });

  it('sets importedAt timestamp', () => {
    const result = parseFFCSV(sampleCSV);

    expect(result.observations[0].importedAt).toBeDefined();
    expect(new Date(result.observations[0].importedAt).getTime()).not.toBeNaN();
  });
