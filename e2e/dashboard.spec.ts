...
test.describe('Dashboard - With Data', () => {
  ...
  test('shows calendar component', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Cycle Calendar/i })).toBeVisible({
      timeout: 10000,
    });
...
test.describe('Dashboard - Loading States', () => {
  test('shows loading spinner while data loads', async ({ page }) => {
    ...
    await expect(page.getByText(/No predictions yet|Unified Fertile Window|Loading/i)).toBeVisible({
      timeout: 10000,
    });
  });
});
